...
function keepSelectedShapesInViewport(app) {
  ...
  const bounds = Utils.getCommonBounds(shapes.map(shape => app.getShapeUtil(shape).getBounds(shape)));
...
export function useSessionStore() {
  const [app, setApp] = useState(/** @type {import("@tldraw/tldraw").TldrawApp | null} */ (null));
  ...
  const handleChanges = useCallback(
    ...
    [applyStateToBoard, amIWhiteboardOwner, zoomToFit],
  );
  ...
  const onChangePage = useCallback(
    debounce((_app, shapes, bindings) => {
      if (isEmptyObject(shapes) && isEmptyObject(bindings)) return;

      Object.values(shapes).forEach(shape => {
        hmsActions.sessionStore.set(`whiteboard.${SHAPE_KEY}.${shape.id}`, shape);
      });

      Object.values(bindings).forEach(binding => {
        hmsActions.sessionStore.set(`whiteboard.${BINDING_KEY}.${binding.id}`, binding);
      });
