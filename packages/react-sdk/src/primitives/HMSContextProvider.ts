...
function createHMSContext() {
  ...
  const HMSRoomProvider: React.FC<PropsWithChildren<HMSRoomProviderProps>> = ({
    children,
    actions,
    store,
    notifications,
  ...
  const useHMSStatsStore = makeHMSStatsStoreHook(HMSContext);
   ...
   * ```
   */
  const useHMSVanillaNotifications = () => {
    const HMSContextConsumer = useContext(HMSContext);
    if (!HMSContextConsumer) {
      throw new Error(hooksErrorMessage);
    }
   ...
   */
  const useHMSNotifications = (type?: HMSNotificationTypes | HMSNotificationTypes[]) => {
    const HMSContextConsumer = useContext(HMSContext);
...
export interface ContextCreationResult {
  HMSRoomProvider: React.FC<PropsWithChildren<HMSRoomProviderProps>>;
  useHMSStore: <StateSlice>(
    selector: StateSelector<HMSStore, StateSlice>,
    equalityFn: EqualityChecker<StateSlice>,
  ) => StateSlice;
  useHMSActions: () => HMSActions;
  useHMSNotifications: (type?: HMSNotificationTypes | HMSNotificationTypes[]) => HMSNotification | null;
  useIsHMSStatsOn: () => boolean;
  useHMSVanillaStore: () => any;
  useHMSVanillaNotifications: () => any;
  useHMSStatsStore: <StateSlice>(
...
export const CreateContextProvider = () => {
  const Contexts = new Map<string, ContextCreationResult>();
  return (type: string) => {
    if (Contexts.has(type)) {
      return Contexts.get(type);
    }
    const contextCreationResult = createHMSContext();
    Contexts.set(type, contextCreationResult);
