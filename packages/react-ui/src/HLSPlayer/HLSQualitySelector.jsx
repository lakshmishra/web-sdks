...
export function HLSQualitySelector({ levels, onQualityChange, selection, isAuto }) {
    ...
    <Dropdown.Root open={qualityDropDownOpen} onOpenChange={value => setQualityDropDownOpen(value)}>
      ...
      {levels.length > 0 && (
        <Dropdown.Content sideOffset={5} align="end" css={{ height: 'auto', maxHeight: '$96', w: '$64' }}>
          ...
          {levels.map(level => {
            return (
              <Dropdown.Item onClick={() => onQualityChange(level)} key={level.url}>
                <Text css={{ flex: '1 1 0' }}>{getQualityText(level)}</Text>
                {!isAuto && level.width === selection?.width && level.height === selection?.height && (
                  <CheckCircleIcon />
                )}
              </Dropdown.Item>
            );
...
const getQualityText = level =>
  `${Math.min(level.height, level.width)}p (${(Number(level.bitrate / 1000) / 1000).toFixed(2)} Mbps)`;
